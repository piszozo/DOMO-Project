...
The RPM spec file provided here is for Heptacoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://heptacoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Heptacoin git repository](https://github.com/heptacoin/heptacoin) but are not part of
...
Currently only `Source10` is used, for the example `heptacoin.conf` file.
...
in the heptacoin git repository. At present only an SVG version of the Heptacoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the heptacoin
...
needed when 0.12.1 is released, a proper fix is already in the Heptacoin git
master branch.
...
idea to build Heptacoin Core against a static version of that library compiled
...
The `heptacoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `heptacoin-server` package. At this point, most RPM based
...
The `heptacoin-server` package should have SELinux support. How to properly do
...
Heptacoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
4. I renamed the `heptacoin` package that contains the Qt GUI to `heptacoin-core` as
