...
*Setup instructions for a Gitian build of Heptacoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Heptacoin
...
- [Building Heptacoin Core](#building-heptacoin-core)
...
Clone the git repositories for heptacoin and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/heptacoin/heptacoin.core.git
...
in the heptacoin repository under 'Fetch and create inputs' to install sources which require
...
To build Heptacoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the heptacoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/heptacoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/heptacoin/heptacoin
...
For example:
```bash
URL=https://github.com/laanwj/heptacoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit heptacoin=${COMMIT} --url heptacoin=${URL} ../heptacoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit heptacoin=${COMMIT} --url heptacoin=${URL} ../heptacoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit heptacoin=${COMMIT} --url heptacoin=${URL} ../heptacoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the heptacoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../heptacoin/contrib/gitian-descriptors/*|sort|uniq )
...
HEPTAPATH=/some/root/path/heptacoin
SIGPATH=/some/root/path/heptacoin-detached-sigs

./bin/gbuild --url heptacoin=${HEPTAPATH},signature=${SIGPATH} ../heptacoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/heptacoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/heptacoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/heptacoin-osx-build.assert
```
